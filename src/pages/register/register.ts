...
export class RegisterPage {
  ...
  ionViewDidLoad() {
    console.log(
      "Header=>" +
        ...
        "px" +
        "Footer=>" +
        document.getElementById("footer-container").offsetHeight +
        "px"
    );
    ...
    window.addEventListener("keyboardWillShow", (event) => {
      ...
      console.log(
        "Header2=>" + headerHeight + "px" + "Footer=>" + footerHeight + "px"
      );
      ...
      document
        ...
        .setAttribute(
          "style",
          "margin-top:0px; margin-bottom: 0px; padding-bottom: " +
            footerHeight +
            "px;"
        );
    });
  ...
  onKeyupExistingUser() {
    ...
    this.validations_form_new_user.reset({
      new_student_name: this.loginData.new_student_name,
      school_access_code: this.loginData.school_access_code,
    });
  }
  ...
  onKeyupNewUser() {
    ...
    this.validations_form_existing_user.reset({
      existing_student_name: this.loginData.existing_student_name,
      existing_student_mfk_id: this.loginData.existing_student_mfk_id,
    });
  }
  ...
  async signIn() {
    if (!this.isFormSubmitted) {
      ...
      if (
        this.loginData.existing_student_name &&
        this.loginData.existing_student_name != "" &&
        this.loginData.existing_student_mfk_id &&
        this.loginData.existing_student_mfk_id != ""
      ) {
        console.log(
          "ExistingUser==>" + this.validations_form_existing_user.valid
        );
        ...
        loader.present();
        this.usersList = await this.databaseprovider
          .getUserDetails()
          .then((res) => res);
        ...
        if (this.usersList.length > 4) {
          ...
          this.dataService.studentLogin(this.loginData).then(
            (result) => {
              ...
              if (this.responseData.returnStatus != 0) {
                ...
                this.dataService
                  .getStudentStar(this.responseData)
                  .then(async (result) => {
                    this.responseData1 = result;
                    console.log(this.responseData1);
                    if (this.responseData1.returnStatus != 0) {
                      this.responseData.starCount =
                        this.responseData1.starCount;
                    } else {
                      this.responseData.starCount = "0";
                    }
                    this.userList = await this.databaseprovider
                      .selectUserById(this.responseData.studentId)
                      .then((res) => res);
                    if (this.userList.length == 0) {
                      this.databaseprovider
                        .addUser(this.responseData)
                        .then((data) => {
                          console.log("Users added to local db.");
                          this.databaseprovider
                            .updateOtherUserLockStatus(
                              this.responseData.studentId
                            )
                            .then((data) => {
                    ...
                    this.navCtrl.push("UserConfirmPage", {
                      userType: "old",
