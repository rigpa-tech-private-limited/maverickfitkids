...
@IonicPage()
@Component({
  selector: 'page-fit-spot-exercise',
  templateUrl: 'fit-spot-exercise.html',
})
export class FitSpotExercisePage {
  ...
  exerciseType: any = "";
  exerciseLists: any;
  exerciseList: any = { "insAudioPath": "", "audioPath": "", "exercisePath": "", "imageVideoName": "", "exerciseDescription": "", "workoutId": "" };
  ...
  nextSlide() {
    ...
    if ((this.currentSlide + 1) == (this.exerciseLists.length)) {
      ...
      this.exerciseList.workoutId = this.exerciseLists[(this.currentSlide + 1)].workoutId;
  ...
  onSearch() {
    this.openSearchModal('FitSpotPopupPage');
  }

  openSearchModal(pageName) {
  ...
  onFinish(){
    this.dataService.markStarForStudent(this.userDetails, 13).then((result) => {
      ...
      if (this.responseData.returnStatus != 0) {
        this.navCtrl.setRoot("AcceptanceCodePage", { "starList": this.responseData.starList,"fromPage":"fitspot" });
  ...
  showAbout() {
    this.openModal('AboutFitSpotPopupPage');
  }
}
