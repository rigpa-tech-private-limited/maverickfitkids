...
@IonicPage()
@Component({
  selector: 'page-everyday-exercise',
  templateUrl: 'everyday-exercise.html',
  animations: [
    trigger('shift', [
...
export class EverydayExercisePage {
    ...
    this.storage.get('userDetails')
      .then((res: any) => {
        if (res) {
          ...
          loader.present();
          this.dataService.getStudentExerciseDetails(this.userDetails).then((result) => {
            loader.dismiss();
            this.responseData = result;
            console.log(this.responseData);
            if (this.responseData.returnStatus != 0) {
              let cusid_ele = document.getElementsByClassName('everyday-exercise-container');
              for (let i = 0; i < cusid_ele.length; ++i) {
                let item = cusid_ele[i];
                item.setAttribute("style", "opacity: 1;");
              }
              this.exerciseLists = this.responseData.exerciseList;
              ...
              if (this.exerciseLists[0].audioPath != null && this.exerciseLists[0].audioPath != '') {
                this.exerciseList.audioPath = (this.exerciseLists[0].audioPath).replace('/maverick/Directory/Music/', AppConfig.SITE_URL + 'maverick/Directory/Music/');
  ...
  toggleTTS() {
    if (this.exerciseList.insAudioPath != null && this.exerciseList.insAudioPath != '') {
      if (this.isSpeaking) {
        this.stopSpeak();
      } else {
        this.startSpeak();
  ...
  toggleAudio() {
    if (this.exerciseList.audioPath != null && this.exerciseList.audioPath != '') {
      if (this.isPlaying) {
        this.stopAudio();
      } else {
        this.startAudio();
  ...
  openModal(pageName) {
    ...
    modal.onDidDismiss(data => {
      if (data.slideAction == 'finish') {
        ...
        this.dataService.markStarForStudent(this.userDetails, 4).then((result) => {
          this.responseData = result;
        ...
        this.initializeBackButtonCustomHandler();
        if (this.exerciseLists[1].insAudioPath != null && this.exerciseLists[1].insAudioPath != '') {
          this.exerciseList.insAudioPath = (this.exerciseLists[1].insAudioPath).replace('/maverick/Directory/Music/', AppConfig.SITE_URL + 'maverick/Directory/Music/');
          this.instruction_icon = 'assets/imgs/instruction_icon_play.png';
        } else {
          this.exerciseList.insAudioPath = "";
          this.instruction_icon = 'assets/imgs/instruction_icon_gray.png';
        ...
        this.insAudio = this.insaudio.nativeElement;
        this.insAudio.src = this.exerciseList.insAudioPath;
        if (this.exerciseLists[1].audioPath != null && this.exerciseLists[1].audioPath != '') {
          this.exerciseList.audioPath = (this.exerciseLists[1].audioPath).replace('/maverick/Directory/Music/', AppConfig.SITE_URL + 'maverick/Directory/Music/');
          this.music_icon = 'assets/imgs/music_icon_play.png';
        } else {
          this.exerciseList.audioPath = "";
          this.music_icon = 'assets/imgs/music_icon_gray.png';
        ...
        this.musicAudio = this.musicaudio.nativeElement;
        this.musicAudio.src = this.exerciseList.audioPath;
        if (this.exerciseLists[1].fileType == '2') {
        ...
        this.exerciseList.imageVideoName = this.exerciseLists[1].imageVideoName;
        if(this.exerciseLists[1].exerciseDescription!=null){
          this.exerciseList.exerciseDescription = (this.exerciseLists[1].exerciseDescription).replace(/(?:\r\n|\r|\n)/g, '<br>');
        } else {
          this.exerciseList.exerciseDescription = '';
        }
        this.goToSlide(this.currentSlide + 2);
        this.videoPlay();
      }
